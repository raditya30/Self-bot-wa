...
const bgimg = (`https://telegra.ph/file/9d839573c7b513e9f3f77.jpg`)
blocked = [];
...
const starts = async (client = new WAConnection()) => {
  ...
  client.browserDescription = ["Eagle", "Chrome", "3.0"];
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "add" && !mem.includes(client.user.jid)) {
        ...
        var buff = await getBuffer(pp_user)
        }
        buttons = [
      ...
      if (anu.action == "promote") {
        ...
        let buffer = await getBuffer(
          `https://api-yogipw.herokuapp.com/api/promote?name=${anu_user}&msg=selamat%20menjadi%20admin&mem=${groupAdmins.length}&picurl=${ppimg}&bgurl=${bgimg}`
        );
      ...
      if (anu.action == "demote") {
        ...
        let buffer = await getBuffer(
          `https://api-yogipw.herokuapp.com/api/demote?name=${anu_user}&msg=yahahaha didemote&mem=${groupAdmins.length}&picurl=${ppimg}&bgurl=${bgimg}`
        );
